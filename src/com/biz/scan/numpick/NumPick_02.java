...
public class NumPick_02 {
	 ...
	 * 코드가 만들어진다.
	 * 
	 * 이떄, 클래스에 정의된 method들이 공통으로 사용할 
	 * 변수들은 member 변수 영역에 선언한다.
	 * 
	 *  member 변수 영역에 선언된 변수중에 
	 *  Primetive(기본형변수 int, float. String, boolean)들은
	 *  자동으로 초기화가 되어 사용준비가 된다.
	 *  
	 *  하지만 reference(참조형변수 배열, 객체 등)들은 자동으로
	 *  초기화가 되지 않아 사용준비가 되지 않는다.
	 */
	...
	/*
	 * member 변수 영역에 선언된 참조형변수들을 사용할수 있도록
	 * 초기화(생성) 코드는 생성자 메서드에 그 코드를 작성해 주어야 한다.
	 */
